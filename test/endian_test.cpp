...
TEST(EndianTest, Test) {
  auto littleEndianData = *accat::auxilia::read_as_bytes<uint32_t>(
      "Z:/luce/data/image.bin");

  auto bigEndianData =
      *accat::auxilia::read_as_bytes<uint32_t, std::endian::big>(
          "Z:/luce/data/image-big_endian.bin");

  auto little_endian_byte_span = std::span{
      reinterpret_cast<const std::byte *>(littleEndianData.data()),
      littleEndianData.size()};
  auto big_endian_byte_span = std::span{
      reinterpret_cast<const std::byte *>(bigEndianData.data()),
      bigEndianData.size()};
