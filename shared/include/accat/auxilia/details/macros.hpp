...
#if __has_include(                                                   \
    <stacktrace>) && defined(AC_UTILS_DEBUG_ENABLED) && defined(_WIN32)
#  include <stacktrace>
...
#    define AC_UTILS_DEBUG_LOGGING_SETUP(                            \
        _exec_, _level_, _msg_, ...)                                 \
      ::spdlog::set_level(spdlog::level::_level_);                   \
      ::spdlog::set_pattern("[\033[33m" #_exec_                      \
                            ":\033[0m %^%5l%$] %v");                 \
...
#define AC_SPDLOG_INITIALIZATION(_exec_, _log_level_)                \
  ...
  static           /* <- msvc can't get through this.*/              \
      const auto AC_SPDLOG_INITIALIZATION =                          \
          [](void) /* static constexpr <- msvc can't get through     \
                      this.*/                                        \
      -> ::std::nullptr_t {                                          \
    ...
    ::std::cout << ::std::flush;                                     \
    ::std::cerr << ::std::flush;                                     \
    AC_UTILS_DEBUG_LOGGING(                                          \
        info, "\033[36mspdlog framework initialized.\033[0m")        \
    AC_UTILS_DEBUG_LOGGING_SETUP(                                    \
        _exec_, _log_level_, "Debug mode enabled")                   \
    return nullptr;                                                  \
  }();
// clang-format off
...
#  warning "MSVC traditional preprocessor was used. no additional debug info will be provided. to enable MSVC's new preprocessor, add compiler flag `/Zc:preprocessor`."
...
#  include <iostream>
#  define AC_UTILS_TODO_(...)                                        \
    ::std::cerr << std::format("TODO: " #__VA_ARGS__)                \
                << ::std::endl;                                      \
...
#  pragma warning(                                                   \
      disable : 4244) // conversion from 'int' to 'char' warning
#else
...
template <class Fun_>
AC_STATIC_CALL_OPERATOR inline constexpr auto
operator*(_utils_defer_helper_struct_, Fun_ f_)
    -> _utils_deferrer_<Fun_> {
  return {f_};
}
} // namespace ::detail
#define AC_DEFER                                                     \
  const auto AC_UTILS_EXPAND_COUNTER(_utils_defer_block_at) =  \
      accat::auxilia::detail::                                  \
          _utils_defer_helper_struct_{} *[&]()
#ifdef defer
...
#define AC_BITMASK_OPS(_bitmask_)                                    \
  inline constexpr _bitmask_ operator&(                              \
    ...
    return (static_cast<_bitmask_>(                                  \
        static_cast<_intergral_type_>(_left_) &                      \
