...
struct invocation_slot_error final : error {
  const char *what() const noexcept override {
    return "rocket: One of the slots has raised an exception during the signal "
           "invocation.";
  }
};
#endif
...
template <class T> struct intrusive_ptr final {
  ...
  intrusive_ptr(intrusive_ptr &&p) noexcept : ptr{p.ptr} {
    p.ptr = nullptr;
  }
...
template <class T, class U>
ROCKET_NODISCARD inline intrusive_ptr<T>
static_pointer_cast(intrusive_ptr<U> const &p) noexcept {
  return intrusive_ptr<T>{static_cast<T *>(p.get())};
}

template <class T, class U>
ROCKET_NODISCARD inline intrusive_ptr<T>
const_pointer_cast(intrusive_ptr<U> const &p) noexcept {
  return intrusive_ptr<T>{const_cast<T *>(p.get())};
}

template <class T, class U>
ROCKET_NODISCARD inline intrusive_ptr<T>
dynamic_pointer_cast(intrusive_ptr<U> const &p) noexcept {
  return intrusive_ptr<T>{dynamic_cast<T *>(p.get())};
}

template <class T, class U>
ROCKET_NODISCARD inline intrusive_ptr<T>
reinterpret_pointer_cast(intrusive_ptr<U> const &p) noexcept {
...
public:
  template <class U> struct iterator_base final {
    ...
    iterator_base(iterator_base &&i) noexcept : element{std::move(i.element)} {}
  ...
  iterator
  insert(iterator const &pos, size_type count, value_type const &value) {
    iterator iter{end()};
  ...
  template <class Iterator> void append(Iterator ibegin, Iterator iend) {
    insert(end(), ibegin, iend);
  }
...
struct connection_base<thread_unsafe_policy>
  ...
  void disconnect() noexcept {
    if (prev != nullptr) {
      ...
      // To mark a connection as disconnected, just set its prev-link to null
      // but leave the next link alive so we can still traverse through the
      // connections if the slot gets disconnected during signal emit.
      prev = nullptr;
    }
  }
...
struct connection_scope final {
  connection_scope(void *base,
                   bool is_thread_safe,
                   thread_local_data *th) noexcept
      : th{th}, prev{th->current_connection},
...
struct connection {
  ...
  ROCKET_NODISCARD bool is_connected() const noexcept {
    if (base != nullptr) {
      if (is_thread_safe) {
        auto safe{
            static_cast<detail::connection_base<thread_safe_policy> *>(base)};
        return std::launder(safe)->is_connected();
      } else {
        auto unsafe{
            static_cast<detail::connection_base<thread_unsafe_policy> *>(base)};
...
struct scoped_connection_container final {
  ...
  operator+=(std::initializer_list<connection> list) {
    for (auto const &connection : list) {
      append(connection);
    }
    return *this;
...
private:
  ...
  scoped_connection_container &
  operator=(scoped_connection_container const &) = delete;
...
private:
  ...
  operator=(scoped_connection_blocker const &) = delete;
...
struct timer_queue final {
  ...
  connection set_interval(slot_type slot,
    ...
    connection_base *base = make_link(tail,
                                      std::move(slot),
                                      std::move(expires_at),
  ...
  template <class Instance,
            class Class,
            class R,
            class Rep = unsigned long,
            class Period = std::milli>
  connection set_interval(Instance &object,
                          R (Class::*method)(),
                          std::chrono::duration<Rep, Period> const &interval) {
  ...
  connection set_interval(Instance &object,
                          std::chrono::duration<Rep, Period> const &interval) {
    connection c{
        set_interval<Rep, Period>([&object] { (object.*Method)(); }, interval)};
    if constexpr (std::is_convertible_v<Instance *, trackable *>) {
      static_cast<trackable &>(object).add_tracked_connection(c);
    }
    return c;
  ...
  connection set_timeout(Instance &object,
                         std::chrono::duration<Rep, Period> const &timeout) {
    connection c{
        set_timeout<Rep, Period>([&object] { (object.*Method)(); }, timeout)};
    if constexpr (std::is_convertible_v<Instance *, trackable *>) {
      static_cast<trackable &>(object).add_tracked_connection(c);
    }
    return c;
  ...
  bool
  dispatch(std::chrono::time_point<std::chrono::steady_clock> execute_until) {
#  ifndef ROCKET_NO_EXCEPTIONS
    bool error{false};
#  endif
  ...
  dispatch(std::chrono::time_point<std::chrono::steady_clock> execute_until) {
    ...
    while (itr != end) {
      ...
      if (execute_until != std::chrono::time_point<std::chrono::steady_clock>{})
        ROCKET_UNLIKELY {
          // check if we already spent the maximum allowed time executing
          // callbacks
          if (execute_until <= std::chrono::steady_clock::now()) {
            break;
          }
...
template <class Rep = unsigned long, class Period = std::milli>
inline connection
set_interval(std::function<void()> slot,
             std::chrono::duration<Rep, Period> const &interval) {
...
template <auto Method, class Rep = unsigned long, class Period = std::milli>
inline connection
set_interval(std::chrono::duration<Rep, Period> const &interval) {
  return detail::get_timer_queue()->template set_interval<Method, Rep, Period>(
      interval);
}
...
template <class Instance,
          ...
          class Period = std::milli>
inline connection
set_interval(Instance &object,
             R (Class::*method)(),
             std::chrono::duration<Rep, Period> const &interval) {
  return detail::get_timer_queue()
      ->template set_interval<Instance, Class, R, Rep, Period>(
...
set_interval(Instance &object,
             std::chrono::duration<Rep, Period> const &interval) {
  return detail::get_timer_queue()
      ->template set_interval<Method, Instance, Rep, Period>(object, interval);
}
...
template <class Rep = unsigned long, class Period = std::milli>
inline connection
set_timeout(std::function<void()> slot,
            std::chrono::duration<Rep, Period> const &timeout) {
...
template <auto Method, class Rep = unsigned long, class Period = std::milli>
inline connection
set_timeout(std::chrono::duration<Rep, Period> const &timeout) {
...
template <class Instance,
          ...
          class Period = std::milli>
inline connection
set_timeout(Instance &object,
            R (Class::*method)(),
            std::chrono::duration<Rep, Period> const &timeout) {
  return detail::get_timer_queue()
      ->template set_timeout<Instance, Class, R, Rep, Period>(
...
inline connection set_interval(Instance &object,
                               R (Class::*method)(),
                               unsigned long interval_ms) {
...
template <class Instance, class Class, class R>
inline connection
set_timeout(Instance &object, R (Class::*method)(), unsigned long timeout_ms) {
...
struct signal<R(Args...), Collector, ThreadingPolicy> final {
  ...
  connection connect(slot_type slot,
    ...
    std::scoped_lock<shared_lock_state> guard{lock_state};
    connection_base *base = make_link(first ? head->next : tail,
                                      std::move(slot)
#ifndef ROCKET_NO_QUEUED_CONNECTIONS
                                          ,
                                      tid
  ...
  connection connect(Instance &object,
                     connection_flags flags = direct_connection) {
    connection c{connect(
        [&object](Args const &...args) { return R((object.*Method)(args...)); },
        flags)};
    if constexpr (std::is_convertible_v<Instance *, trackable *>) {
      static_cast<trackable &>(object).add_tracked_connection(c);
    }
              ...
              if (current->is_queued())
                ROCKET_UNLIKELY {
                  if constexpr (std::is_void_v<R>) {
                        ...
                        [current, args = detail::make_tuple(args...)] {
                          if (current->is_connected())
                            ROCKET_LIKELY {
                              ...
                              functional_connection *conn = std::launder(
                    ...
                    detail::get_call_queue()->put(current->get_tid(),
                                                  std::move(task));
                  } else {
                    // If we are calling a queued slot, and our signal requires
                    // a return value we actually have to block the thread until
                    // the slot was dispatched
                    std::packaged_task<void()> task(
                        [current,
                         &collector,
...
private:
  ...
  functional_connection *make_link(connection_base *l,
                                   slot_type slot
#ifndef ROCKET_NO_QUEUED_CONNECTIONS
                                   ,
...
template <class Instance, class Class, class R, class... Args>
ROCKET_NODISCARD inline std::function<R(Args...)>
